...
const rideSchema = new Schema(
  {
    uniqueRideKey: { type: String, unique: true, index: true },
...
const rideRequestSchema = new Schema(
  {
    // 🔑 Unique key generated by user (same one used in Ride)
    uniqueRideKey: {
      type: String,
      required: true,
      unique: true,
    ...
    status: {
      type: String,
      enum: ["pending", "locked", "assigned", "expired"],
      default: "pending",
      index: true,
    },

    // 🚗 Driver who accepted (only for assigned)
    acceptedBy: {
      type: Schema.Types.ObjectId,
      ref: "driver",
      default: null,
